...
const seadrop_1 = require("./seadrop");
...
function registerPendingHandler(chatId, handler) {
...
bot.use((0, grammy_1.session)({
    initial: session_1.createDefaultSession,
}));
// ─────────────────────────────────────────────────────────────────────────────
...
bot.command("start", async (ctx) => {
    ...
    const chain = (0, chains_1.resolveChain)(sess.settings.activeChain);
    const walletCount = sess.walletAddresses.length;
    const activeSnipes = sess.activeSnipes.filter((s) => s.status === "pending" || s.status === "waiting").length;
    const walletLines = walletCount === 0
    ...
    const text = `🔫 <b>SeaDrop NFT Sniper Bot</b>\n\n` +
...
bot.command("wallets_add", async (ctx) => {
    ...
    if (!chatId)
        return;
    await ctx.reply(`🔑 <b>Send me a private key to add.</b>\n\n` +
        `The key is encrypted in memory and never saved to disk.\n` +
        `Send /cancel_w to abort.`, { parse_mode: "HTML" });
    registerPendingHandler(chatId, async (msgCtx, _next) => {
        ...
        try {
            ...
            const encrypted = (0, session_1.encryptWallet)(key);
            ctx.session.wallets.push(encrypted);
            ctx.session.walletAddresses.push(wallet.address);
            await msgCtx.reply(`✅ <b>Wallet added!</b>\n\nAddress: ${code(wallet.address)}`, { parse_mode: "HTML" });
        }
        catch (err) {
            await msgCtx.reply(`❌ <b>Invalid private key:</b> ${esc(err.message || "Could not parse key")}`);
        }
    });
});
...
bot.command("wallets_view", async (ctx) => {
    ...
    const lines = await Promise.all(sess.walletAddresses.map(async (addr, i) => {
        try {
            const balance = await provider.getBalance(addr);
            return `${i + 1}. ${code(addr)}\n   💰 ${(0, ethers_1.formatEther)(balance)} ETH`;
        }
        catch {
            return `${i + 1}. ${code(addr)}\n   💰 Balance: unknown`;
        }
    }));
    await ctx.reply(`👤 <b>Your Wallets (${sess.walletAddresses.length}):</b>\n\n` + lines.join("\n\n"), { parse_mode: "HTML" });
});
// /wallets_clear — Clear all wallets
...
bot.command("snipe", async (ctx) => {
    ...
    await ctx.reply(`🎯 <b>SeaDrop Snipe Wizard</b>\n\n` +
        `Step 1/6: <b>Contract Address</b>\n` +
        `Send the NFT contract address (0x...).`, { parse_mode: "HTML" });
});
...
bot.command("confirm_snipe", async (ctx) => {
    ...
    const chain = (0, chains_1.resolveChain)(sess.settings.activeChain);
    const rpcUrls = (0, mint_1.resolveRpcUrls)(sess.settings.customRpc, process.env.ADDITIONAL_RPC_URLS || "");
    await ctx.reply("🔍 Building mint plan from on-chain data...");
    const plan = await (0, seadrop_1.buildMintPlan)(rpcUrls[0], wizard.contractAddress, wizard.quantity);
    if (!plan) {
        await ctx.reply("❌ Could not build mint plan. Contract may not be a SeaDrop collection.");
        sess.snipeWizard = undefined;
        return;
    }
    const maxFee = (0, ethers_1.parseUnits)(wizard.maxFeePerGas, "gwei");
    const maxTip = (0, ethers_1.parseUnits)(wizard.maxPriorityFee, "gwei");
    ...
    if (wizard.timingMode === "now") {
        ...
        try {
            const results = await (0, mint_1.executeSnipe)(sess.wallets.map((w) => (0, session_1.decryptWallet)(w)), plan, rpcUrls, maxFee, maxTip, 250_000, BigInt(chain?.chainId || chains_1.DEFAULT_CHAIN.chainId), (log) => sess.logs.push(log));
            ...
            await ctx.reply(`📊 <b>Snipe Results</b>\n\n` + resultLines.join("\n\n"), { parse_mode: "HTML", link_preview_options: { is_disabled: true } });
        }
        catch (err) {
        ...
        await ctx.reply(`⏰ <b>Scheduled!</b>\n\n` +
            ...
            `Use /cancel to abort before it fires.`, { parse_mode: "HTML" });
        const timeoutId = setTimeout(async () => {
            ...
            try {
                ...
                if (ctx.chat) {
                    ...
                    await ctx.api.sendMessage(ctx.chat.id, `🚀 <b>Scheduled Snipe Fired!</b>\n\n` + resultLines.join("\n"), { parse_mode: "HTML" });
                }
            }
            catch (err) {
                activeSnipe.status = "failed";
                if (ctx.chat) {
                    await ctx.api.sendMessage(ctx.chat.id, `❌ <b>Scheduled snipe failed:</b> ${esc(err.message)}`, { parse_mode: "HTML" });
                }
            }
        }, waitMs);
        activeSnipe._timeoutId = timeoutId;
    }
});
...
bot.command("set_chain", async (ctx) => {
    ...
    if (!chatId)
        return;
    const chainList = chains_1.CHAINS.map((c) => `🔹 ${esc(c.key)} — ${esc(c.name)} (ID: ${c.chainId})`).join("\n");
    await ctx.reply(`📡 <b>Available Networks:</b>\n\n` +
        chainList + "\n\n" +
        `Send the chain key (e.g., ${code("robinhood")})`, { parse_mode: "HTML" });
    registerPendingHandler(chatId, async (msgCtx, _next) => {
        if (!msgCtx.message?.text)
            return;
        const chain = (0, chains_1.resolveChain)(msgCtx.message.text.trim());
        if (!chain) {
...
bot.command("set_rpc", async (ctx) => {
    ...
    if (!chatId)
        return;
    await ctx.reply(`🔗 <b>Set Custom RPC URL</b>\n\n` +
        `Send a full RPC URL, or ${code("default")} to use the chain default.`, { parse_mode: "HTML" });
    registerPendingHandler(chatId, async (msgCtx, _next) => {
        if (!msgCtx.message?.text)
            return;
...
bot.command("status", async (ctx) => {
    const sess = ctx.session;
    const activeSnipes = sess.activeSnipes.filter((s) => s.status === "pending" || s.status === "waiting" || s.status === "firing");
    const snipeLines = activeSnipes.length === 0
        ? "  No active snipes."
        : activeSnipes.map((s) => {
            const countdown = s.scheduledTime && s.status === "waiting"
    ...
    const text = `📊 <b>Status Dashboard</b>\n\n` +
...
bot.command("cancel", async (ctx) => {
    ...
    const pending = sess.activeSnipes.filter((s) => s.status === "pending" || s.status === "waiting");
    if (pending.length === 0) {
    ...
    for (const snipe of pending) {
        ...
        const timeoutId = snipe._timeoutId;
        if (timeoutId) {
...
bot.on("message:text", async (ctx) => {
    ...
    switch (wizard.step) {
        case 1: {
            ...
            const drop = await fetchPublicDropSafe(rpc, text);
            if (drop) {
                await ctx.reply(`✅ <b>SeaDrop Public Drop Found!</b>\n\n` +
                    `Price: ${code((0, ethers_1.formatEther)(drop.mintPrice) + " ETH")}\n` +
                    ...
                    `How many NFTs per wallet? (1–${drop.maxTotalMintableByWallet})`, { parse_mode: "HTML" });
            }
            else {
                await ctx.reply(`⚠️ Could not fetch drop info (may not be SeaDrop or network issue).\n\n` +
                    `Step 2/6: <b>Mint Quantity</b>\n` +
                    `How many NFTs per wallet?`, { parse_mode: "HTML" });
            }
            break;
        }
        case 2: {
            ...
            wizard.quantity = qty;
            wizard.step = 3;
            await ctx.reply(`Step 3/6: <b>Max Fee Per Gas (ceiling)</b>\n\n` +
                `Max gas price you'll tolerate.\n` +
                `Recommended: ${esc(sess.settings.maxFeePerGas)} Gwei\n\n` +
                `Send a value in Gwei, or send ${code("default")} to use the current setting.`, { parse_mode: "HTML" });
            break;
        }
        case 3: {
            ...
            wizard.step = 4;
            await ctx.reply(`Step 4/6: <b>Priority Fee (tip)</b>\n\n` +
        ...
        case 4: {
            ...
            wizard.step = 5;
            await ctx.reply(`Step 5/6: <b>Timing Mode</b>\n\n` +
                `Send ${code("now")} to fire immediately, or ${code("scheduled")} to wait for a specific time.`, { parse_mode: "HTML" });
            break;
        }
        case 5: {
            if (text.toLowerCase() === "now") {
                ...
                wizard.step = 51;
                await ctx.reply(`⏰ <b>Scheduled Mint Time</b>\n\n` +
                    `Send the mint time in ISO format or Unix timestamp.\n` +
                    `Example: ${code("2025-01-15T18:00:00Z")}`, { parse_mode: "HTML" });
            }
            else {
...
// HELPER FUNCTIONS
// ─────────────────────────────────────────────────────────────────────────────
async function showSnipeSummary(ctx, sess) {
    const wizard = sess.snipeWizard;
    const chain = (0, chains_1.resolveChain)(sess.settings.activeChain);
    const timingText = wizard.timingMode === "now"
        ? "🚀 Fire Now"
        : `⏰ Scheduled: ${wizard.scheduledTime}`;
    const text = `📋 <b>Snipe Summary</b>\n\n` +
...
async function fetchPublicDropSafe(rpcUrl, nftContract) {
    try {
        const { fetchPublicDrop } = await Promise.resolve().then(() => __importStar(require("./seadrop")));
        return await fetchPublicDrop(rpcUrl, nftContract);
    }
    catch {
        return null;
    }
